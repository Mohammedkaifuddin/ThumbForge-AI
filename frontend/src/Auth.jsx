  ...
  : `${API_URL}/api/auth/register`;
    ...
    try {
      ...
      if (serverErrors && serverErrors.length > 0) {
        ...
        setError(err.response?.data?.message || 'Authentication failed. Check your data.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="w-full max-w-md bg-slate-800/50 border border-slate-800 p-8 rounded-2xl backdrop-blur-sm shadow-xl">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {isLogin ? 'Sign in to forge high-converting thumbnails' : 'Create your free account to start forging'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Password (min 6 chars)</label>
            <div className="relative">
        ...
        <div className="text-center mt-6">
          <button
            type="button"
            onClick={() => { setIsLogin(!isLogin); setError(''); setSuccessMsg(''); setShowPassword(false); }}
