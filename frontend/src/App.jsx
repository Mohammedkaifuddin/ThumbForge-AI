...
const API_BASE_URL = "https://thumbforge-ai-backend.onrender.com/api/thumbnail";

function App() {
  const [token, setToken] = useState(
    localStorage.getItem("thumbforge_token") || "",
  );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.data.success) {
        fetchHistory();
        if (
          currentImage === history.find((item) => item._id === id)?.imageUrl
        ) {
          setCurrentImage(null);
        }
      }
      ...
      <main className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <section className="lg:col-span-5 space-y-6">
          <div className="bg-slate-800/50 border border-slate-800 p-6 rounded-2xl">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <Wand2 size={18} className="text-indigo-400" /> Configure
              Thumbnail
            </h2>
          ...
          <div className="bg-slate-800/30 border border-slate-800/60 p-4 rounded-2xl flex flex-col items-center justify-center min-h-[260px]">
            {currentImage ? (
              <img
                src={currentImage}
                alt="Live Output"
                className="w-full h-auto aspect-video object-cover rounded-xl border border-slate-700"
              />
            ) : (
              <p className="text-sm text-slate-500">
                Submit a title to generate thumbnail mockups
              </p>
            )}
          </div>
        </section>
        ...
        <section className="lg:col-span-7 space-y-4">
          <h2 className="text-lg font-bold flex items-center gap-2">
            <History size={18} className="text-slate-400" /> Generation Archive
            ({history.length})
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[640px] overflow-y-auto pr-2">
            {history.map((item) => (
              <div
                key={item._id}
                className="bg-slate-800/40 border border-slate-800 rounded-xl overflow-hidden group relative"
              >
                <img
                  src={item.imageUrl}
                  alt={item.title}
